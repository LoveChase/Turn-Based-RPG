var enemyplaytest = {"name":"420 Hazyyyyy","author":"DBGxOsO","created":"2018-01-18","skill":"Beatdown!",
"main":
[
{"name":"Light Dragon","amount":5, "health":100, "mana":100, "strength":100, "agility":100, "speed":100, "defense":100, "attPerTurn":1},
{"name":"Light Dragon","amount":5, "health":100, "mana":100, "strength":100, "agility":100, "speed":100, "defense":100, "attPerTurn":1},
{"name":"Shadow Dragon","amount":5, "health":100, "mana":100, "strength":100, "agility":100, "speed":100, "defense":100, "attPerTurn":1}
],
"extra":[],
"notes":[{"text":"First time KOG. Been playing since day one. Used a Phoenix build for most of my climb the. Switched to this in legend."}],
"url":"/top-decks/january-2018/hazy-flame-br/420-hazyyyyy-by-dbgxoso/"}


$('#play').click(function () {
    $('#playtest').show();
    //From player.js Defines player/enemy object
    player=new Player();
    enemy=new Enemy();
  });
  ...
  let enemydecklist = [];
  let enemyextradecklist = [];
  let enemyhand = [];
  let enemyboard = [];
  let enemycurrentDeck = joinMainExtraDeck(enemyplaytest.main, enemyplaytest.extra);
  
  // Not currently using //
  let enemyencode = window.btoa(JSON.stringify(getExportReadyDeck(currentDeck)));
  
  $('#export').click(function () {
    $('.export').html(`<textarea style="width:320px" class="code">${enemyencode}</textarea>`);
    $('.export').dialog({
      width: 388,
    ...
    for (let card in currentDeck) {
      ...
      if (!found) {
        joined.push({
          ...
          health: currentDeck[card].health,
          mana: currentDeck[card].mana,
          strength: currentDeck[card].strength,
          agility: currentDeck[card].agility,
          speed: currentDeck[card].speed,
          defense: currentDeck[card].defense,
          attPerTurn: currentDeck[card].attPerTurn,
  ...
  function joinMainExtraDeck(main, extra) {
    ...
    for (let card in main) {
      joined.push({
        name: main[card].name,
        health: main[card].health,
        mana: main[card].mana,
        strength: main[card].strength,
        agility: main[card].agility,
        speed: main[card].speed,
        defense: main[card].defense,
        attPerTurn: main[card].attPerTurn,
        deck: 0,
  ...
  function openDeck(deck, extra = false) {
    ...
    for (i in extra ? extradecklist : decklist) {
      if (extra)
      $('#deckmenu').append(`<img style="margin: 1px" src="../img/avatar-player/${extradecklist[i].name.toLowerCase()+'.png'}" width="60px" id="${extradecklist[i].id}"/>`);else
  
      $('#deckmenu').append(`<img style="margin: 1px" src="../img/avatar-player/${decklist[i].name.toLowerCase()+'.png'}" width="60px" id="${decklist[i].id}"/>`);
    }
  ...
  function dealHand(i) {
    ...
    addHand(decklist[i].name, decklist[i].id, decklist[i].health, decklist[i].mana, decklist[i].strength, decklist[i].agility, decklist[i].speed, decklist[i].defense, decklist[i].attPerTurn);
  ...
  function addCardToHandDiv(i, extra) {
    if (extra == 0)
    $('#hand').append(
    `<div id="cardId${decklist[i].id}" class="testcard-slot-row" onclick="GameManager.setPlayer(this)">
    <div class="hand cardMain${decklist[i].id}"><img id="${decklist[i].id}" src="../img/avatar-player/${decklist[i].name.toLowerCase()+'.png'}" alt="${decklist[i].name}"/>
    <h3>${decklist[i].name}</h3>
    <p class="health-player">Health: ${decklist[i].health}</p>
    <p>Mana: ${decklist[i].mana}</p>
    <p>Strength: ${decklist[i].strength}</p>
    <p>Agility: ${decklist[i].agility}</p>
    <p>Speed: ${decklist[i].speed}</p>
    <p>Defense: ${decklist[i].defense}</p>
    <p class="AttPerTurn-player">Attacks per Turn: ${decklist[i].attPerTurn}</p></div>`);
    
    else
    if (extra == 1)
    $('#hand').append(`<div id="cardId${extradecklist[i].id}" class="testcard-slot-row"><div class="hand cardEx${extradecklist[i].id}"><img id="${extradecklist[i].id}" src="../img/avatar-player/${extradecklist[i].name.toLowerCase()+'.png'}" /></div>`);
  ...
  function addHand(i, id, health, mana, strength, agility, speed, defense, attPerTurn) {
    hand.push({
      id: id,
      name: i,
      health: health,
  ...
  function addCardToDeck(c) {
    ...
    decklist.push({
      ...
      health: hand[cardIndexInHand].health,
      mana: hand[cardIndexInHand].mana,
      strength: hand[cardIndexInHand].strength,
      agility: hand[cardIndexInHand].agility,
      speed: hand[cardIndexInHand].speed,
      defense: hand[cardIndexInHand].defense,
      attPerTurn: hand[cardIndexInHand].attPerTurn });
  ...
  function addCardToExtraDeck(c) {
    ...
    extradecklist.push({
      id: hand[cardIndexInHand].id,
      name: hand[cardIndexInHand].name,
      test: hand[cardIndexInHand].test });
      ...
      for (let j = 0; j < Number(deck[i].amount); j++) {if (window.CP.shouldStopExecution(3)) break;
        if (deck[i].deck == 0) {
          decklist.push({
            id: id,
            name: deck[i].name,
            health: deck[i].health,
            mana: deck[i].mana,
            strength: deck[i].strength,
            agility: deck[i].agility,
            speed: deck[i].speed,
            defense: deck[i].defense,
            attPerTurn: deck[i].attPerTurn });
